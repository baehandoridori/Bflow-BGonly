...
- **D. Success Check**: `SuccessCheckCircle`(TodoRow + SceneCard의 TodoCard **양쪽** 원형 체크 대체). `completed` false→true: ring→check scale spring + ring-burst 1회. true→false(되돌리기): 즉시/짧게 해제, **burst·콘페티 없음**. aria-pressed/title/onToggle(stopPropagation)/키보드 보존. reduce→즉시.

...
## v1.44.0 출시 범위 (정직한 스코프 — 코덱스 P2 반영)
**이번 PR(v1.44.0)은 "기존 리테이크를 세트로 가져와 관리"까지 구현한다.** 세트 생성/삭제(컴포지터급) + 자동취합 탭(파트/담당자/진행상태/씬순) + 시안 B 테이블 + 담당 워크플로우·단순 상태(담당자 없는 항목) 처리 + 기존 리테이크 가져오기/빼기 + 진행률/자동완료(resolve·이동·삭제·뷰 모든 경로에서 동기화).

**범위 밖(후속):** 허브에서 **'새 항목 직접 생성'(특히 씬 미지정 '전반' 항목)** 은 핵심 리비전 생성 경로(`addRevision`)의 시그니처 변경(scene-less + set_id 허용)이 필요해, 리뷰 후 별도로 안전하게 한다. 그동안 net-new 리테이크는 **씬 화면에서 평소대로 등록한 뒤 허브에서 '가져오기'로 담는다(2-step)**. '전반'(scene_id NULL) 항목 자체는 데이터·표시·자동취합이 이미 지원되므로, 생성 경로만 추가되면 즉시 동작한다.

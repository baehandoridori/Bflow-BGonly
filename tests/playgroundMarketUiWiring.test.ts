...
test('playground market v3 release metadata stays aligned', () => {
  ...
  assert.equal(packageJson.version, '1.88.1');
  assert.equal(packageLock.version, '1.88.1');
  assert.equal(packageLock.packages[''].version, '1.88.1');
  ...
  assert.deepEqual(
    ...
    ['1.88.1', '1.88.0', '1.87.0', '1.86.0'],
  );

  // 최신 릴리스(1.88.1)는 게임판 크기 조정(넓은 화면 대응).
  assert.equal(updateNotes[0].version, '1.88.1');

...
test('playground market v3 release metadata stays aligned', () => {
  ...
  assert.equal(packageJson.version, '1.88.2');
  assert.equal(packageLock.version, '1.88.2');
  assert.equal(packageLock.packages[''].version, '1.88.2');
  ...
  assert.deepEqual(
    ...
    ['1.88.2', '1.88.1', '1.88.0', '1.87.0'],
  );

  // 최신 릴리스(1.88.2)는 게임판이 창 높이를 꽉 채우도록 개선.
  assert.equal(updateNotes[0].version, '1.88.2');

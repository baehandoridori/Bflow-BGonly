...
import { COMMENT_LIST_BOTTOM_SLACK_PX, COMMENT_LIST_OPENING_WINDOW_MS, commentListScrollAfterSectionGrow } from '../src/utils/commentListAnchor.ts';
...
test('패널을 막 열어 첫 목록이 들어오면 맨 아래로 가는 부드러운 스크롤을 다시 건다 — 안 읽은 댓글·특정 댓글로 이동 중이면 제외', () => {
  assert.equal(commentListScrollAfterSectionGrow({ ...grown, scrollTop: 120, firstLoadAfterMs: 200, jumpingToComment: false }), 'smooth');
...
test('첫 목록이 늦게(1초 넘게) 오면 막 연 패널로 보지 않는다 — 그사이 위로 올려 읽던 사람을 끌어내리지 않는다 (코덱스 4차)', () => {
  ...
  assert.equal(commentListScrollAfterSectionGrow({ ...grown, scrollTop: 120, firstLoadAfterMs: COMMENT_LIST_OPENING_WINDOW_MS, jumpingToComment: false }), 'smooth');

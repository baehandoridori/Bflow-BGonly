...
async function harness(options:{editable?:boolean;completed?:boolean;clampScroll?:boolean;statusFilter?:'all'|'active'|'completed';configure?:(project:GanttProject)=>void}={}) {
  ...
  return {saved,created,captured,released,selections,menus,relocations,chart,task,project,props,render,
    hoverName(title=task.title,focus=false){render();const label=find(row(title),e=>hasClass(e,'gantt-name'));if(focus)label.props.onFocus({currentTarget:barNode});else label.props.onPointerMove(pointer(200));return find(render(),e=>Object.hasOwn(e.props,'hover')).props.hover;},
    ...
    menu(title:string){find(render(),e=>e.props['aria-label']===`${title} 메뉴`).props.onClick({...pointer(0),currentTarget:barNode});},rows(){return elements(render()).filter(e=>hasClass(e,'gantt-row')).map(e=>text(e));},rowCompleted(title:string){return elements(find(render(),e=>hasClass(e,'gantt-row')&&text(e).includes(title))).some(e=>e.props['aria-label']==='완료');},
...
test('main rows keep names and progress while details move into hover and keyboard focus',async()=>{
  const memo='첫 문장\n둘째 문장\n마지막 메모까지 유지';
  ...
  try{
    ...
    assert.equal(elements(tree).some(e=>hasClass(e,'gantt-row-kind')||hasClass(e,'gantt-parent-context')||hasClass(e,'gantt-duration')),false);
    for(const focus of [false,true]){const info=h.hoverName('검증 작업',focus);assert.equal(info.typeLabel,'작업');assert.equal(info.context,'검증 프로젝트 › 진행 그룹');assert.equal(info.task.memo,memo);assert.equal(info.progress,20);assert.equal(info.hasDates,true);}
  }finally{h.dispose();}
});
...
test('empty groups and projects still expose their complete memo without fabricated dates',async()=>{
  ...
  try{const group=h.hoverName('빈 묶음');assert.equal(group.task.memo,'빈 그룹 메모');assert.equal(group.hasDates,false);const project=h.hoverName('검증 프로젝트');assert.equal(project.task.memo,'프로젝트 전체 메모');assert.equal(project.typeLabel,'프로젝트');assert.equal(project.hasDates,false);}finally{h.dispose();}
});

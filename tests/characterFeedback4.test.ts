...
test('피드백 45: 팝업 렌더러도 사용자 디렉터리 IPC 모드를 켠다', () => {
  ...
  const userServiceImport = popup.match(/import \{([\s\S]*?)\} from '@\/services\/userService';/)?.[1] ?? '';
  for (const name of ['loadSession', 'loadUsers', 'setUsersSheetsMode']) {
    assert.match(userServiceImport, new RegExp(`\\b${name}\\b`));
  }

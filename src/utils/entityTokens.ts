 ...
 *  - cut: 컷N/cutN (4a) — ⚠️ 4c에서 #태그로 전환 중. hash 배선 완료 후 정리 단계(Chunk5)에서 제거 예정.
 ...
 *
 * 경로를 먼저 분리한 뒤(중첩 회피) 각 text 조각에서 mention·cut·hash 를 위치순 병합한다
...
function tokenizeTextSegment(text: string, userNames: string[]): EntityToken[] {
  ...
  // hash 를 cut 보다 먼저 수집 — '[#컷5](...)' 처럼 라벨 안에 컷 패턴이 있어도 hash 가 앞선 토큰으로 우선되게.

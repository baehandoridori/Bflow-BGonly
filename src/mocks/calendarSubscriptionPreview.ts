...
type Entry = CalendarFeedStatus & { ownerId: string; hash: string | null };
...
/** Preview preserves the owner, CAS and hash-only contracts; URLs are unreachable. */
export function createCalendarSubscriptionPreview(deps: Dependencies) {
  ...
  function owned(calendarId: string) {
    if (!isCalendarFeedId(calendarId)) throw new Error('캘린더 식별자가 올바르지 않습니다.');
    const actor = { ...deps.actor() };
    if (!actor.id || deps.owner(calendarId) !== actor.id) throw new Error('캘린더 소유자만 구독을 관리할 수 있습니다.');
    return actor;
  }
  function checkSession(calendarId: string, origin: { id: string; epoch: number }) {
    const actor = owned(calendarId);
  ...
  function statusLocked(calendarId: string): CalendarFeedStatus {
    ...
    return row
      ? { calendarId, enabled: row.enabled, issuedAt: row.enabled ? row.issuedAt : null, revision: row.revision, preview: true }
  ...
  return {
    ...
    async manage(request: CalendarFeedRequest): Promise<CalendarFeedResult> {
      ...
      const origin = owned(request.calendarId);
      ...
      const result = await deps.lock(async () => {
        const actor = checkSession(request.calendarId, origin);
        ...
        const row: Entry = {
          ...
          revision: crypto.randomUUID(), hash, preview: true,
        };
        const rows = read();
      ...
      // that interval must not deliver the previous user's one-time bearer URL.
      checkSession(request.calendarId, origin);
      return result;
    },
  };
}

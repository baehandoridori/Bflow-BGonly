...
async function deleteEventForToken(
  ...
  try {
    ...
    // 낙관적 알림 뒤 다른 창이 stale snapshot을 다시 담았을 수 있으므로 sender cache
    // 존재 여부와 무관하게 영속 삭제 완료를 정확한 calendar+event로 한 번 더 확정한다.
    broadcastCommittedGoogleDelete(calId, actualId);

...
import {
  ...
  resolveMarketBoundaryState,
  type MarketLoadingVariant,
} from './marketDataBoundaryView';

interface MarketDataBoundaryProps {
  children: ReactNode;
  loadingVariant: MarketLoadingVariant;
}
...
export function MarketDataBoundary({ children, loadingVariant }: MarketDataBoundaryProps) {
  ...
  const boundaryState = resolveMarketBoundaryState({
    hasVisibleSnapshot: visible !== null,
    error,
  });
  ...
  if (boundaryState === 'loading') {
    return (
      ...
      >
        ...
        <MarketLoadingSkeleton variant={loadingVariant} />
      </div>
    );
  }

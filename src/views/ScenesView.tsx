...
export function ScenesView() {
  ...
  // 'all' 모드: BG+ACT 씬 머지
  // 1차: sceneId 완전 일치
  // 2차: 남은 씬들 간 normalizeSceneIdKey 매칭 — "ac001" ↔ "a001" 같은 접두사 변형 대응
  const mergedScenes = useMemo((): MergedScene[] => {
    ...
    // 1차: ACT 중 완전 일치만 병합하고, 나머지는 따로 모은다
  ...
  // 통합 상세 모달은 mergedScenes 최신 참조를 유지해야 한다.
  useEffect(() => {
  ...
  const actToBgImageMap = useMemo(() => {
    ...
    for (const bgScene of bgSibling.scenes) {
      const key = normalizeSceneIdKey(bgScene.sceneId);
      if (!key) continue;
    ...
    <div ref={gridRef} className="relative flex gap-3 min-h-full">
                ...
                if (selectedDepartment === 'all') {
                  const collectFromMerged = (prefix: 'bg' | 'act') => {
                    ...
                    selectedSceneIds.forEach((id) => {
                      ...
                      const mergedKey = id.slice(prefix.length + 1);
      ...
      {detailScene && detailSceneIdx !== null && (() => {
        ...
        const counterpart = (() => {
          ...
          const match = otherPart.scenes.find((scene) => normalizeSceneIdKey(scene.sceneId) === targetKey);
      ...
      {detailMerged && selectedDepartment === 'all' && (() => {
        const curIdx = mergedScenes.findIndex((scene) => scene.sceneId === detailMerged.sceneId);
        ...
        return (
          <UnifiedSceneDetailModal
            ...
            onAddDept={async (dept) => {
              ...
              const existing = targetPart.scenes.find((scene) => scene.sceneId === detailMerged.sceneId);
              if (existing) {

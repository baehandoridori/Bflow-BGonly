  ...
  if (variant === 'chip') {
    return (
      <button
        onClick={handleJump}
        title="씬 상세 모달 열기"
        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md border border-accent/30 bg-accent/10 text-[11px] font-bold text-accent-sub hover:bg-accent/18 transition-colors cursor-pointer"
      >
  ...
  if (variant === 'action') {
    return (
      <button
        onClick={handleJump}
        title="씬 상세 모달 열기"
        className="inline-flex min-h-[28px] items-center gap-1.5 rounded-md border border-accent/30 bg-accent/[0.08] px-2.5 text-[11px] font-bold text-accent-sub hover:border-accent/45 hover:bg-accent/[0.14] transition-colors cursor-pointer"
      >

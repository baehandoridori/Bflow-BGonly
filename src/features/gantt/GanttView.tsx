...
export function GanttView() {
  ...
  return <section className={`gantt ${navOpen?'':'nav-hidden'}`}>
    ...
    {calendarImportOpen&&user&&<CalendarImportDialog key={user.id} actorId={user.id} initialProjectId={targetProject?.id} onClose={()=>setCalendarImportOpen(false)} onImported={(project,count,skipped)=>{setCalendarImportOpen(false);setStatusFilter('all');setHidden(ids=>ids.filter(id=>id!==project.id));select(project.id,null);setImportNotice(`${count}개 일정을 가져왔어요.${skipped?` 이미 가져온 ${skipped}개는 제외했어요.`:''}`);}}/>}

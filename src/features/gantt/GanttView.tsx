...
export function GanttView() {
  ...
  const run=async(action:()=>Promise<unknown>,message='저장했습니다.')=>{try{const result=await action();if(result!==false)setNotice(message);}catch(e){setNotice((e as Error).message);}};
  ...
  const relocate=async(source:GanttProject,task:GanttTask,target:GanttProject,targetTaskId:string|null,position:RelocationPosition)=>{
    ...
    setCollapsed(ids=>ids.filter(id=>!expanded.has(id)));setClosedSpaces(ids=>ids.filter(id=>id!==saved.spaceId));setHidden(ids=>ids.filter(id=>id!==saved.id));setSelectedProject(saved.id);setSelected([task.id]);setInspecting(true);
    return true;
  };
  const complete=async(project:GanttProject,t:GanttTask|null)=>{
    ...
    if(!t){
      ...
      if(t.progressMode==='scenes')setNotice('씬 연결을 유지하고, 이 작업의 완료 상태는 직접 관리하도록 변경했습니다.');
    }
    setContext(null);
  };
  ...
  const deleteSpace=async(space:GanttSpace)=>{
    ...
    setSpaceDialog(undefined);setNotice('빈 폴더를 삭제했습니다. 실행 취소로 되돌릴 수 있습니다.');
  };
  ...
  return <section className={`gantt ${navOpen?'':'nav-hidden'}`}>
    {navOpen&&<nav className="gantt-nav" aria-label="간트 폴더와 프로젝트"><div className="gantt-nav-heading"><strong>폴더와 프로젝트</strong><button title="목록 접기" onClick={()=>setNavOpen(false)}><X size={14}/></button></div><input className="gantt-search" aria-label="프로젝트·작업 검색" placeholder="프로젝트·작업 검색" value={search} onChange={e=>setSearch(e.target.value)}/><div className="gantt-nav-actions"><button onClick={()=>setHidden([])}>전체 표시</button><button onClick={()=>setHidden(snapshot.projects.map(p=>p.id))}>모두 끄기</button>{prior&&<button onClick={()=>{setHidden(prior);setPrior(null);}}>이전 선택</button>}</div>
      <GanttTree spaces={snapshot.spaces} projects={effectiveProjects} userId={user?.id||''} users={users} selectedProject={selectedProject} selected={selected} closedSpaces={closedSpaces} collapsed={collapsed} hidden={hidden} filter={statusFilter} search={search} worker={worker} onToggleFolder={id=>setClosedSpaces(ids=>ids.includes(id)?ids.filter(x=>x!==id):[...ids,id])} onToggleBranch={id=>setCollapsed(ids=>ids.includes(id)?ids.filter(x=>x!==id):[...ids,id])} onToggleVisibility={id=>setHidden(ids=>ids.includes(id)?ids.filter(x=>x!==id):[...ids,id])} onSelect={treeSelect} onFolderSettings={space=>afterDraft(()=>setSpaceDialog(space))} onMenu={(project,task,x,y)=>setContext({project:snapshot.projects.find(p=>p.id===project.id)!,task:task?snapshot.projects.find(p=>p.id===project.id)!.tasks.find(t=>t.id===task.id)!:null,x,y})}/>
    ...
    <main className="gantt-main"><div className="gantt-heading"><div>{!navOpen&&<button onClick={()=>setNavOpen(true)}><ChevronRight size={16}/> 프로젝트</button>}<h1>타임라인</h1></div><div><button disabled={!state.canUndo||pending} title="실행 취소 (Ctrl+Z)" aria-label="실행 취소" onClick={()=>void run(()=>state.undo())}><Undo2 size={15}/></button><button disabled={!state.canRedo||pending} title="다시 실행 (Ctrl+Shift+Z)" aria-label="다시 실행" onClick={()=>void run(()=>state.redo())}><Redo2 size={15}/></button></div></div>
      ...
      <div className="gantt-insertion-hint">{targetProject?'새 작업 위치: '+targetProject.name+(task?(task.kind==='group'?' › '+task.title:task.parentId?' › '+(targetProject.tasks.find(t=>t.id===task.parentId)?.title||''):''):''):''}</div>
      ...
      {loading&&!snapshot.spaces.length?<div className="gantt-empty">프로젝트를 불러오고 있습니다.</div>:<GanttCanvas projects={visibleProjects} selected={selected} statusFilter={statusFilter} worker={worker} collapsed={collapsed} names={names} onRelocate={(source,task,target,targetTaskId,position)=>void run(()=>relocate(source,task,target,targetTaskId,position),'작업 위치를 변경했습니다.')} canEdit={project=>!pending&&editable(project)} onCollapse={id=>setCollapsed(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id])} onSelect={select} onPatch={(p,t,patch)=>void run(()=>patchTask(p,t,patch))} onAdd={(p,parent,start,end)=>void run(()=>addTask(p,'task',parent,start,end))} onMenu={(project,task,x,y)=>{setContext({project:snapshot.projects.find(p=>p.id===project.id)!,task:task?snapshot.projects.find(p=>p.id===project.id)!.tasks.find(t=>t.id===task.id)!:null,x,y});}}/>}
      <footer className="gantt-footer"><span>{pending?'저장 중…':'프로젝트별 변경을 저장합니다.'}</span><span>빈 곳 드래그로 화면 이동 · 작업 만들기 모드에서 범위 드래그 · ⋯ 메뉴로 완료·삭제</span></footer>
    </main>

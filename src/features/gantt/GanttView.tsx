...
import { LinkedCalendarPanel } from './LinkedCalendarPanel';
import { CalendarImportDialog } from './CalendarImportDialog';
import { expandLinkedCalendarProjects, linkedCalendarVisibleRange, type LinkedCalendarRange } from './linkedCalendarRecurrence';
...
export function GanttView() {
  ...
  const [calendarRange,setCalendarRange]=useState(()=>linkedCalendarVisibleRange(localDate(),900,48));
  const showCalendarRange=useCallback((range:LinkedCalendarRange)=>setCalendarRange(previous=>previous.from===range.from&&previous.to===range.to?previous:range),[]);
  ...
  const [calendarImportOpen,setCalendarImportOpen]=useState(false),[importNotice,setImportNotice]=useState('');
  useEffect(()=>{
    ...
    setPreferencesLoadedFor(preferenceKey);setCalendarImportOpen(false);setImportNotice('');setSelectedProject(null);setSelected([]);setInspecting(false);setNotice('');
  ...
  const editable=(p:GanttProject)=>{const current=snapshot.projects.find(row=>row.id===p.id);return !!user&&!!current&&!current.calendarLink&&canEditProject(snapshot,user.id,current);};
  const calendarProjects=useMemo(()=>expandLinkedCalendarProjects(snapshot.projects,calendarRange),[snapshot.projects,calendarRange]);
  const effectiveProjects=useMemo(()=>calendarProjects.map(p=>({...p,tasks:p.tasks.map(t=>{const draft=draftProgress[`${p.id}:${t.id}`];if(draft!==undefined)return {...t,progress:draft,completed:draft===100};if(t.progressMode!=='scenes'||!t.sceneLinks.length)return t;const linked=t.sceneLinks.flatMap(l=>episodes.find(ep=>ep.episodeNumber===l.episodeNumber)?.parts.find(part=>part.sheetName===l.sheetName&&part.department===l.department)?.scenes.filter(s=>s.sceneId===l.sceneId)||[]);const progress=linked.length?Math.round(linked.reduce((sum,s)=>sum+sceneProgress(s),0)/linked.length):0;return {...t,progress,completed:progress===100};})})),[calendarProjects,episodes,draftProgress]);
  const canonicalProject=snapshot.projects.find(p=>p.id===selectedProject);
  const p=canonicalProject?.calendarLink?effectiveProjects.find(p=>p.id===selectedProject):canonicalProject,task=p?.tasks.find(t=>t.id===selected[0])||null;
  ...
  return <section className={`gantt ${navOpen?'':'nav-hidden'}`}>
    {navOpen&&<nav className="gantt-nav" aria-label="간트 폴더와 프로젝트"><div className="gantt-nav-heading"><strong>폴더와 프로젝트</strong><button title="목록 접기" onClick={()=>setNavOpen(false)}><X size={14}/></button></div><input className="gantt-search" aria-label="프로젝트·작업 검색" placeholder="프로젝트·작업 검색" value={search} onChange={e=>setSearch(e.target.value)}/><div className="gantt-nav-actions"><button onClick={()=>setHidden([])}>전체 표시</button><button onClick={()=>setHidden(snapshot.projects.map(p=>p.id))}>모두 끄기</button>{prior&&<button onClick={()=>{setHidden(prior);setPrior(null);}}>이전 선택</button>}</div>
      <GanttTree spaces={snapshot.spaces} projects={effectiveProjects} userId={user?.id||''} users={users} selectedProject={selectedProject} selected={selected} closedSpaces={closedSpaces} collapsed={treeCollapsed} hidden={hidden} filter={statusFilter} search={search} worker={worker} onToggleFolder={id=>setClosedSpaces(ids=>ids.includes(id)?ids.filter(x=>x!==id):[...ids,id])} onToggleBranch={id=>setTreeCollapsed(ids=>ids.includes(id)?ids.filter(x=>x!==id):[...ids,id])} onToggleVisibility={id=>setHidden(ids=>ids.includes(id)?ids.filter(x=>x!==id):[...ids,id])} onSelect={treeSelect} onFolderSettings={space=>afterDraft(()=>{if(space.calendarLink){const project=snapshot.projects.find(p=>p.spaceId===space.id);if(project){setSelectedProject(project.id);setSelected([project.id]);setInspecting(true);}}else setSpaceDialog(space);})} onMenu={(project,task,x,y)=>setContext({project:project.calendarLink?project:snapshot.projects.find(p=>p.id===project.id)!,task:task?(project.calendarLink?task:snapshot.projects.find(p=>p.id===project.id)!.tasks.find(t=>t.id===task.id)!):null,x,y})}/>
      <details className="gantt-hierarchy-help"><summary>폴더 · 프로젝트 · 그룹이란?</summary><p>폴더는 프로젝트를 정리하고 공유하는 곳입니다.</p><p>프로젝트 안에 작업을 만들고, 필요할 때 그룹으로 묶습니다. 그룹 없이 작업만 만들어도 됩니다.</p><p>폴더 → 프로젝트 → 그룹(선택) → 작업</p></details><button className="gantt-nav-add" onClick={()=>setSpaceDialog(null)}><FolderPlus size={14}/> 폴더 만들기</button><button className="gantt-nav-add" disabled={!snapshot.spaces.some(s=>!s.calendarLink&&(s.ownerId===user?.id||s.members.some(m=>m.userId===user?.id&&m.canEdit)))} onClick={()=>setProjectDialog(true)}><Plus size={14}/> 프로젝트 만들기</button>
    </nav>}
    <main className="gantt-main"><div className="gantt-heading"><div>{!navOpen&&<button onClick={()=>setNavOpen(true)}><ChevronRight size={16}/> 프로젝트</button>}<h1>타임라인</h1></div><div><span className="gantt-save-indicator" role="status" aria-live="polite">{pending?'저장 중…':''}</span><button disabled={!state.canUndo||pending} title="실행 취소 (Ctrl+Z)" aria-label="실행 취소" onClick={()=>void run(()=>state.undo())}><Undo2 size={15}/></button><button disabled={!state.canRedo||pending} title="다시 실행 (Ctrl+Shift+Z)" aria-label="다시 실행" onClick={()=>void run(()=>state.redo())}><Redo2 size={15}/></button></div></div>
      ...
      {importNotice&&<div className="gantt-import-notice" role="status"><span>{importNotice}</span><button aria-label="가져오기 안내 닫기" onClick={()=>setImportNotice('')}><X size={14}/></button></div>}

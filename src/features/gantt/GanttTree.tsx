import { Check, ChevronRight, Circle, Diamond, Eye, EyeOff, Folder, FolderOpen, Layers, Link2, MoreHorizontal, Settings2 } from 'lucide-react';
    ...
    {[false,true].map(shared=><div key={String(shared)} role="none"><h3>{shared?'공유 폴더':'내 폴더'}</h3><ul role="group">
      {spaces.filter(space=>space.shared===shared).map(space=>{
        ...
        return <li role="none" key={space.id}><div role="treeitem" tabIndex={0} aria-label={space.name} aria-expanded={open} className="gantt-tree-row folder">
          ...
          {space.calendarLink&&<span className="gantt-linked-badge" title="원본 캘린더와 일정·공유 설정 자동 연결"><Link2 size={11}/>연결</span>}{space.shared&&<GanttShareTooltip space={space} users={props.users}/>}
          {(space.calendarLink||space.ownerId===props.userId)&&<button type="button" className="gantt-tree-menu" aria-label={`${space.name} 설정`} onClick={()=>props.onFolderSettings(space)}><Settings2 size={13}/></button>}
          ...
          return <li role="none" key={project.id}><div role="treeitem" tabIndex={0} aria-expanded={expanded} aria-selected={active} aria-label={`${project.name} · 프로젝트`} className={`gantt-tree-row project ${active?'selected':''} ${off?'off':''}`} onKeyDown={event=>{if(event.target===event.currentTarget&&event.key==='Enter')props.onSelect(project.id,null);}}>
            ...
            {project.calendarLink&&<span className="gantt-linked-badge" title="원본 일정을 열어 편집합니다"><Link2 size={11}/></span>}{project.completed&&<Check size={11} aria-label="완료"/>}

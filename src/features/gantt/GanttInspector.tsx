...
export function GanttInspector(props: GanttInspectorProps) {
  ...
  const select = (label: string, value: string, options: Array<{value: string;label: string;disabled?: boolean}>, onChange: (value: string) => void, locked = false) => <GanttSelect label={label} value={value} options={options} onChange={onChange} disabled={disabled || locked} />;

  return <aside className="gantt-inspector" aria-label={task ? '작업 상세' : '프로젝트 상세'}>
    <div className="gantt-inspector-sticky">
      <div className="gantt-inspector-header"><div><span className="gantt-inspector-eyebrow">{[props.folderName,project.name,task?.parentId?project.tasks.find(row=>row.id===task.parentId)?.title:undefined].filter(Boolean).join(' › ')}</span><h2>{task ? group?'그룹 상세':milestone?'마일스톤 상세':'작업 상세' : '프로젝트 설정'}</h2></div><button className="gantt-button" type="button" aria-label="상세 닫기" onClick={()=>void close()}><X size={17}/></button></div>
    ...
    <form onSubmit={event=>{event.preventDefault();flush();}} className="gantt-inspector-form">
      <fieldset disabled={disabled} className="gantt-inspector-fields">
        {taskDraft ? <>
          <label className="gantt-field">제목<input name="title" value={taskDraft.title} onChange={event=>changeTask({title:event.target.value},false)} onBlur={flush} maxLength={200} required/></label>
          ...
          <details className="gantt-inspector-fold"><summary>종류와 상위 그룹</summary><div className="gantt-inspector-fold-content"><label className="gantt-field">종류{select('종류',taskDraft.kind,[{value:'task',label:'작업',disabled:hasChildren},{value:'group',label:'그룹'},{value:'milestone',label:'마일스톤',disabled:hasChildren}],value=>changeTask({kind:value as GanttTask['kind']}))}</label><label className="gantt-field">상위 그룹{select('상위 그룹',taskDraft.parentId||'',[{value:'',label:'프로젝트 바로 아래'},...parentChoices.map(parent=>({value:parent.id,label:parent.title}))],value=>changeTask({parentId:value||null}))}</label>{hasChildren&&<p className="gantt-inspector-hint">하위 작업이 있는 그룹은 종류를 바꿀 수 없습니다.</p>}</div></details>
          ...
          <label className="gantt-field">프로젝트 이름<input name="name" value={projectDraft.name} onChange={event=>changeProject({name:event.target.value},false)} onBlur={flush} maxLength={200} required/></label>
    ...
    <div className="gantt-inspector-actions">{task&&<div className="gantt-inspector-order">{([['up','위로',ArrowUp],['down','아래로',ArrowDown],['indent','하위로',ArrowRight],['outdent','상위로',ArrowLeft]] as const).map(([direction,label,Icon])=><button key={direction} type="button" className="gantt-button" aria-label={label} title={label} disabled={actionDisabled} onClick={()=>onMove(direction)}><Icon size={14}/></button>)}</div>}{group&&<button className="gantt-button" type="button" disabled={actionDisabled||props.canAddChild===false} onClick={onAddChild}><Plus size={14}/>하위 작업</button>}<button className="gantt-button" type="button" disabled={actionDisabled} onClick={onComplete}><Check size={14}/>{(props.completed??(task?task.completed||task.progress===100:project.completed))?'다시 열기':'완료 표시'}</button><button className="gantt-button gantt-inspector-delete" type="button" disabled={actionDisabled} onClick={onDelete}><Trash2 size={14}/>삭제</button></div>
  </aside>;
}

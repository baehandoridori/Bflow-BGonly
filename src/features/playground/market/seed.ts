...
const STOCK_INPUTS: StockSeed[] = [
  { id: 'jbbj', name: 'JBBJ', symbol: 'JBBJ', character: '콘텐츠 스튜디오', description: '우리 팀의 작품과 캐릭터를 만드는 스튜디오예요.', referencePriceWon: 1842, previousCloseWon: 1700, reason: '새 프로젝트 공개 소식 뒤 관심이 크게 늘었어요.' },
  { id: 'youtube', name: 'YouTube', symbol: 'YT', character: '영상 플랫폼', description: '전 세계 시청자에게 영상을 전하는 플랫폼이에요.', referencePriceWon: 1260, previousCloseWon: 1222, reason: '신규 채널 성장 소식이 긍정적으로 반영됐어요.' },
  { id: 'meta-comedy', name: '메타코미디', symbol: 'META', character: '코미디 콘텐츠', description: '코미디언과 새로운 웃음 콘텐츠를 만드는 회사예요.', referencePriceWon: 920, previousCloseWon: 944, reason: '신작 공개 전 관망하는 움직임이 많았어요.' },
  { id: 'netflix', name: 'Netflix', symbol: 'NFLX', character: '글로벌 스트리밍', description: '다양한 나라의 작품을 보여주는 스트리밍 서비스예요.', referencePriceWon: 1540, previousCloseWon: 1528, reason: '특별한 소식 없이 보통 범위에서 움직였어요.' },
  { id: 'adobe', name: 'Adobe', symbol: 'ADBE', character: '창작 소프트웨어', description: '그림과 영상 제작에 쓰는 창작 도구를 만들어요.', referencePriceWon: 770, previousCloseWon: 779, reason: '업데이트 평가가 엇갈리며 소폭 내렸어요.' },
  { id: 'wacom', name: 'Wacom', symbol: 'WACM', character: '창작 장비', description: '그림을 그리는 펜과 태블릿을 만드는 회사예요.', referencePriceWon: 430, previousCloseWon: 412, reason: '새 태블릿 공개 소식 뒤 관심이 늘었어요.' },
  { id: 'slack', name: 'Slack', symbol: 'WORK', character: '팀 커뮤니케이션', description: '팀이 대화하고 자료를 나누는 협업 도구예요.', referencePriceWon: 610, previousCloseWon: 610, reason: '특별한 소식 없이 보통 범위에서 움직였어요.' },
  { id: 'google-drive', name: 'Google Drive', symbol: 'GDRV', character: '파일 협업', description: '팀 파일을 보관하고 함께 편집하게 해주는 서비스예요.', referencePriceWon: 505, previousCloseWon: 498, reason: '협업 기능 개선 소식이 작게 반영됐어요.' },
];
...
function makeSeries(stock: StockSeed, period: MarketPeriod): PricePoint[] {
  ...
  const start = stock.referencePriceWon - scaledChange;
  const values = [
    start,
  ...
  return values.map((priceWon, index) => ({
...
export function createMarketPreviewSeed(): MarketSnapshot {
  ...
  const pricesAreValid = stocks.every((stock) => (
    Number.isSafeInteger(stock.referencePriceWon)
    && stock.referencePriceWon > 0
    && Number.isSafeInteger(stock.previousCloseWon)
    && stock.previousCloseWon > 0
    && stock.series.today.at(-1)?.priceWon === stock.referencePriceWon
  ));
  if (!pricesAreValid || summary.totalAssetsWon !== 0 || summary.monthlyTotalPnlWon !== 0) {

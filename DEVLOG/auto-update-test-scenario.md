...
> 기준 버전: `v1.22.19`
> 목적: 토스트 감지뿐 아니라 실제 installer helper 적용, 최신 버전 열림, 업데이트 내역 표시까지 검증한다. `v1.22.10 → v1.22.13` directory swap 실패 찌꺼기가 남은 PC도 복구 경로에 포함한다.
...
목표: 구버전 앱을 새로 켰을 때 스플래시에서 빠르게 버전을 확인하고, 업데이트가 있으면 최신 버전으로 열린다.
...
- 10초 안에 준비되면 installer helper 진행 창이 보이고, 설치 후 최신 앱이 열린다.
...
- installer helper 후 최신 버전 앱이 열린다.
- 좌하단 버전이 배포 예정 버전으로 바뀐다.
- `swap.log`에 `[installer-main]`, `[installer]`, `installer apply OK`, `BFLOW relaunched`가 남는다.

---

## 3-1. 일반 종료 후 다음 실행 자동 적용 테스트

목표: 업데이트 준비 완료 상태에서 앱을 그냥 종료하면 설치 파일을 유지하고, 다음 실행 때 자동으로 적용한다.

절차:

1. 2번 테스트에서 업데이트 준비 완료 상태를 만든다.
2. `지금 업데이트`를 누르지 않고 앱을 완전히 종료한다.
3. 바탕화면/시작 메뉴의 `B flow` 바로가기로 앱을 다시 실행한다.

기대 결과:

- 종료 직후에는 앱이 자동으로 다시 켜지지 않는다.
- 다음 실행 스플래시에서 installer helper가 실행된다.
- 설치 후 최신 BFLOW.exe가 자동으로 열린다.
- `swap.log`에 `[installer-main]`, `[installer]`, `installer apply OK`, `BFLOW relaunched`가 남는다.
...
- 시작 시 업데이트 성공
- 앱 실행 중 업데이트 감지 성공
- `지금 업데이트` 또는 일반 종료 후 다음 실행에서 실제 버전 상승
...
- `v1.22.15` 배포 후 토스트/모달이 뜨고, `지금 업데이트` 후 앱이 `v1.22.15`로 열린다.

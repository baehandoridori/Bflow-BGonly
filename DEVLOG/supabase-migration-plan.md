...
### Phase M-6: 컷오버 (반나절 다운타임)

```
실행 시점: 퇴근 후 또는 주말

순서:
  1. 팀에 공지: "지금부터 반나절 앱 사용 중단"
  2. 마이그레이션 스크립트 실행 (sheets → Supabase)
     - sheets-to-json.ts 실행
     - json-to-supabase.ts 실행
     - verify-migration.ts 실행 → 정합성 확인
  3. 새 빌드 배포 (공유 드라이브에 복사)
  4. 한솔님이 먼저 테스트
     - 씬 체크박스 토글 → 다른 PC에서 즉시 반영 확인
     - 댓글 작성 → 다른 PC에서 즉시 확인
     - 에피소드/씬 추가/삭제 동작 확인
  5. 문제 없으면 팀에 공지: "사용 재개"
  6. 문제 있으면 → 롤백 (이전 빌드로 복원, 시트 데이터 그대로 유지됨)
```

-- ============================================================
-- 배플레이그라운드 2048 forward migration
-- date: 2026-07-14
--
-- 기존 아케이드 v1 스키마를 유지하면서 2048만 허용한다.
-- 새 테이블·인덱스·원장 kind를 만들지 않고 game CHECK와 두 hardened RPC만 교체한다.
-- 실행 순서: 앱 배포 전에 이 migration을 먼저 적용한다.
--
-- ACCEPTED TEST-ONLY THREAT MODEL:
-- - 현재 Electron main의 canonical SessionManager와 anon key를 단일 사용자 가상 포인트 시험 경계로 둔다.
-- - RPC는 DB의 정확한 배한솔 name + Slack ID 한 행을 다시 확인하지만 Supabase Auth 소유권 증명은 아니다.
-- - 팀 공개나 실제 가치·민감 데이터로 확장하기 전에는 검증된 서버 인증이 필수다.
-- ============================================================

BEGIN;

ALTER TABLE public.playground_game_runs
  DROP CONSTRAINT IF EXISTS playground_run_game_valid;
ALTER TABLE public.playground_game_runs
  ADD CONSTRAINT playground_run_game_valid CHECK (
    game_id IN ('snake', 'tetris', 'sudoku', '2048')
  );
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'games', jsonb_build_object(
      ...
      '2048', jsonb_build_object(
        'myBestScore', COALESCE((
          SELECT MAX(r.score) FROM public.playground_game_runs AS r
          WHERE r.user_id = p_user_id::text AND r.game_id = '2048'
        ), 0),
        'myWeeklyBestScore', COALESCE((
          SELECT MAX(r.score) FROM public.playground_game_runs AS r
          WHERE r.user_id = p_user_id::text AND r.game_id = '2048'
        ...
        'todayRewardedRuns', (
          SELECT COUNT(*) FROM public.playground_game_runs AS r
          WHERE r.user_id = p_user_id::text AND r.game_id = '2048' AND r.reward_points > 0
            AND (r.created_at AT TIME ZONE 'Asia/Seoul')::date = v_today
        ),
        'totalRuns', (
          SELECT COUNT(*) FROM public.playground_game_runs AS r
        ...
        'maxGoldenEaten', COALESCE((
          SELECT MAX((r.meta ->> 'goldenEaten')::bigint) FROM public.playground_game_runs AS r
          WHERE r.user_id = p_user_id::text AND r.game_id = '2048' AND r.meta ->> 'goldenEaten' ~ '^[0-9]+$'
        ), 0),
        'maxLineClear', COALESCE((
          SELECT MAX((r.meta ->> 'maxLineClear')::bigint) FROM public.playground_game_runs AS r
          WHERE r.user_id = p_user_id::text AND r.game_id = '2048' AND r.meta ->> 'maxLineClear' ~ '^[0-9]+$'
        ), 0),
        'maxLevel', COALESCE((
          SELECT MAX((r.meta ->> 'levelReached')::bigint) FROM public.playground_game_runs AS r
          WHERE r.user_id = p_user_id::text AND r.game_id = '2048' AND r.meta ->> 'levelReached' ~ '^[0-9]+$'
        ), 0),
        'leaderboardAll', COALESCE((
          SELECT jsonb_agg(
            jsonb_build_object('userId', top.user_id, 'name', top.name, 'score', top.score, 'at', top.created_at)
...
BEGIN
  ...
  CASE p_kind
    ...
    WHEN 'game-start' THEN
      ...
      IF v_game_id NOT IN ('snake', 'tetris', '2048') THEN
    ...
    WHEN 'game-finish' THEN
      ...
      END IF;
      IF v_game_id = '2048' AND (v_score < 0 OR v_score > 10000000) THEN
        RAISE EXCEPTION '2048 score is out of range' USING ERRCODE = '22023';
      END IF;
      ...
      ELSIF v_game_id = 'tetris' THEN
        ...
        v_reward := CASE v_grade
          ...
          ELSE 0
        END;
      ELSIF v_game_id = '2048' THEN
        v_grade := CASE
          WHEN v_score >= 35000 THEN 'platinum'
          WHEN v_score >= 18000 THEN 'gold'
          WHEN v_score >= 8000 THEN 'silver'
          WHEN v_score >= 3000 THEN 'bronze'
          ELSE 'none'
        END;
        v_reward := CASE v_grade
        ...
        END;
      ELSE
        RAISE EXCEPTION 'game grade configuration is invalid' USING ERRCODE = '22023';
      END IF;
...
-- 생성 직후 기본 PUBLIC 권한을 회수하고 v1과 동일하게 anon에만 공개한다.

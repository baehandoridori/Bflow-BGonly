...
## 2026-07-13: 장기 exact 시계열은 render 동기 계산과 현재 진행 봉을 분리한다

### 증상

- 600일 exact 일봉을 render-time `useMemo`에서 한 번에 계산해 화면이 약 11초 멈췄다.
- 과거 계산을 분 단위로 묶어 비동기화한 뒤에는 현재 일봉의 거래량이 최대 59초 늦었고, KST 자정 직후에는 전날 데이터가 오늘 일봉에 섞였다.

### 교훈

- 장기 exact 집계는 완료 일봉 cache를 재사용하면서 작은 단위로 yield하고, 최신 구간부터 점진 표시해 render thread의 긴 동기 차단을 피한다.
- 완료된 과거 일봉과 현재 진행 일봉을 분리한다. 과거는 progressive 계산하고 현재 일봉 하나만 실제 `nowMs`로 다시 계산해야 OHLCV 전체가 함께 갱신된다.
- UTC 일봉과 KST 화면 범위가 만나는 현재 봉의 시작은 `max(UTC day start, visible range start)`로 잘라 이전 KST 일자 데이터가 섞이지 않게 한다.
- 비동기 차트 결과는 요청 key와 함께 저장하고 현재 종목·기간·이벤트 key가 일치할 때만 표시해 이전 요청의 한 프레임 노출을 막는다.

### 추가 교훈 — 범위 경계·인과 key·모든 간격을 같은 계약으로 다룬다
...
## 2026-08-26: 캘린더 변경은 전용 갱신 경로와 실제 수신부를 함께 검증한다

### 증상

- 캘린더 테이블의 `data-change`가 일반 전체 새로고침 분기로 흘러, 일정 하나를 저장해도 앱 전체가 다시 불러와질 수 있었다.
- 송신부 주석에는 캘린더 변경 전달이 남아 있었지만, 수신부 실제 분기에 해당 소스가 빠져 비공개 일정 변경이 화면에 반영되지 않는 문제가 2026-08까지 잠복했다.

### 교훈

- 전체 새로고침 분기가 있는 앱에 새 테이블을 추가할 때는 같은 PR에서 `data-change` 제외 목록도 갱신해, 해당 데이터가 전용 재조회 경로만 타는지 확인한다.
- 실시간 전달을 추가하거나 고칠 때는 송신부 주석이나 이벤트 이름만 믿지 말고, 수신부가 해당 테이블·소스를 실제로 분기해 재조회하는지 mock/preview 테스트로 확인한다.

---

## 2026-08-27: 임시 일정 이관은 화면이 아니라 메인 프로세스가 끝까지 책임진다

### 증상

- 원본 일정 삭제가 확인되기 전에도 replacement를 보관할 수 있어 원본과 공개 replacement가 함께 남을 수 있었다.
- 로그인 전환·앱 종료·renderer 새로고침 사이에 continuation이 사라지면, 이미 만들어진 replacement를 정리할 주체가 없어질 수 있었다.
- 테스트 모드가 이관 상태만 흉내 내고 알림과 세션 전환 경계를 다르게 처리하면 실제 앱에서만 생기는 중복·노출 문제를 놓칠 수 있었다.

### 교훈

- replacement는 원본 삭제가 성공하거나 결과가 불확실해 보존해야 한다는 메인 판단이 난 뒤에만 보관한다. 그 전에는 exact target 삭제만 허용한다.
- 이관 receipt의 전환·종료·만료 정리는 renderer callback에 맡기지 않는다. 메인 프로세스가 작업 시작 전부터 추적하고, 세션 publish 전과 종료 drain 안에서 terminal 상태까지 직접 정리한다.
- preview/mock도 같은 provisional→source 결과→keep/delete 규칙과 수신자별 알림 흐름을 행동 테스트로 고정한다.

## 2026-09-05: 간트 빠른 편집은 입력 전체를 하나의 변경으로 저장한다

- 빠른 편집 안의 색상 선택은 제목·메모·날짜와 같은 draft에 보관한다. 색상만 즉시 저장하며 메뉴를 닫으면 작성 중인 내용이 유실된다.
- 낙관적 revision 증가만으로 편집창을 닫지 않는다. 원격 수정은 저장 시 revision으로 검사하고, 실패한 초안은 남겨야 한다.
- 날짜축을 프로젝트별로 다시 계산할 때 현재 보던 날짜가 바뀌지 않도록 원점과 스크롤을 보정한다. 시간 일정·그룹·연결선은 같은 좌표 계산을 사용한다.
- 시각적 시안의 별도 tooltip 대신 앱의 공통 glassStyle, 커서 추종, 지연·취소 동작을 확인한다.

## 2026-09-05: 생성 대상은 화면 선택과 같은 상태를 사용한다

- 간트 선택 프로젝트와 생성용 드롭다운을 별도 상태로 두면 다른 프로젝트에 작업을 만들 수 있다. 추가 대상은 같은 선택 상태를 사용하고 그룹의 자식/작업의 다음 형제를 명시적으로 결정한다.
- 저장 중 잠금은 편집 권한과 구분한다. pending 때문에 선택 목록을 지우거나 보기 전용 안내로 바꾸지 않는다.
- 기존 엔티티 수정은 배열의 기존 자리를 교체한다. filter+push는 저장할 때마다 프로젝트/폴더 순서를 바꾼다.
- 그룹 기간은 실제 하위 작업으로 계산하며 빈 그룹에 기본 작업 기간을 보여주지 않는다.

## 2026-09-05: anon 키 앱의 권한 모델은 서버가 검증하는 세션 없이는 무의미하다

- 호출자가 `p_actor_id` 를 보내는 RPC 는 anon 키만 있으면 누구든 다른 사람 행세를 할 수 있다. 소유자/편집자/보기 같은 ACL 을 새로 도입할 때는 서버가 발급·검증하는 세션 토큰(`app_login` → `gantt_session_*`)으로 신원을 확정하고, 테이블과 내부 함수는 anon 에서 완전히 회수한다.
- 세션 토큰이 있어도 `users.password` 를 anon 이 읽을 수 있으면 "비밀번호 읽기 → 로그인 → 토큰" 으로 우회된다. 컬럼 권한 축소는 `select *` 클라이언트를 통째로 깨므로(Supabase 문서 명시) 배포 순서(앱 먼저 → DB 나중)를 마이그레이션 파일 머리에 적고, 앱의 `users` 조회는 명시 컬럼으로 바꿔 둔다.
- 이 프로젝트의 default privileges 는 새 테이블/함수에 anon·authenticated EXECUTE/CRUD 를 자동 부여한다. 내부 전용 객체는 만든 직후 `REVOKE ... FROM PUBLIC, anon, authenticated` 를 명시한다.
- 로그인 실패를 `RAISE EXCEPTION` 으로 끝내면 실패 횟수 기록이 같은 트랜잭션과 함께 롤백된다. 인증 실패는 `{ok:false}` 반환값으로 돌려 잠금 카운터가 커밋되게 한다.
- 행 내용을 anon 구독자에게 흘리지 않으려면 테이블을 Realtime publication 에 넣지 말고, statement 트리거에서 `realtime.send(...)` 로 "변경됐다" 신호만 공개 채널에 보낸다. 앱은 신호를 받고 RPC 로 다시 읽는다.
- 라이브 SQL 검증은 마이그레이션 + smoke 를 `BEGIN … ROLLBACK` 한 배치로 먼저 돌린 뒤 적용하고, 적용 후 같은 smoke 를 다시 돌린다. 검증용 사용자는 트랜잭션 안에서만 만든다.


### 2026-09-06: 간트의 저장 검증과 실제 사용 흐름은 따로 확인

- 완료 직후 기본 필터에서 숨기고 상세까지 닫으면 삭제로 오해한다. 기본 전체 보기에서 완료 상태를 표시하고 명시적 필터로만 숨긴다.
- 폴더는 프로젝트 묶음과 공유 범위, 그룹은 프로젝트 내부 작업 묶음이다. 생성·상세·삭제 화면에서 이 차이를 설명한다.
- 프로젝트 다시 열기는 개별 작업의 완료·진행률 초기화와 분리한다. 그룹 표시와 버튼은 같은 하위 작업 상태를 사용한다.
- 손 커서는 실제 이동 가능한 곳에만 사용한다. 화면 이동 드래그는 데이터 저장·작업 생성을 일으키지 않는지 마우스로 확인한다.


### 2026-09-06: 자동 저장과 드래그는 클릭 이후의 입력까지 보호한다

- 입력 blur 직후 추가 버튼이 비활성화되면 첫 클릭이 사라진다. 자체 저장 중인 초안은 flush를 기다린 뒤 원래 클릭을 이어서 처리한다.
- 저장 대기 중 상세 선택이나 같은 상세의 입력이 바뀔 수 있다. 최종 선택 직전에도 초안 guard와 선택·계정 세대를 다시 확인한다.
- 트리와 차트는 정렬값이 같을 때도 같은 안정 순서를 사용한다. 선택창 높이는 명목 최대 높이가 아니라 실제 남은 화면 공간으로 제한한다.
- 프로젝트 간 이동은 두 프로젝트와 공유 폴더 revision을 한 트랜잭션으로 검사한다. 실패·재시도·실행 취소에서 어느 한쪽만 반영되어서는 안 된다.

### 2026-09-06: 표시 정보도 정본 경계를 따른다

- 연결 일정의 색은 캘린더 색으로 덮어쓰지 말고 작업→그룹→프로젝트 상속 결과를 main·preview·renderer·실패 복구에서 같은 규칙으로 유지한다.
- 보기 전환은 별도 인덱스만 바꾸지 말고 사용자가 보고 있던 기준 날짜를 함께 전달한다.
- 0분 마일스톤의 최소 표시 높이는 화면에만 적용하고, 저장값이나 크기 조절에 가짜 기간을 만들지 않는다.
- 밝은 태그 색은 테두리·배경으로 남기고 작은 본문 글자는 테마 잉크로 읽기 쉽게 한다. 부모 팝업의 바깥 클릭 판단에는 포털 메뉴 영역도 포함한다.

### 2026-09-06: 간트 탐색과 상세 정보는 작업 공간을 차지하지 않게 분리한다

- 사이드바 트리의 접힘은 탐색 상태이고 차트의 접힘은 작업 표시 상태다. 서로 별도 저장하고 생성·이동 결과를 보여줄 때도 사이드바를 임의로 펼치지 않는다.
- 종류·상위 위치·성공 메시지를 상시 늘어놓으면 일정이 보이는 공간이 줄어든다. 상시는 이름·진행·필수 조작에 집중하고 긴 메모는 끝까지 읽을 수 있는 호버로 제공한다.
- 입력과 공용 드롭다운에 서로 다른 글자 배율을 적용하면 상세창의 폰트가 불균형해진다. 실제 앱의 사용자 글자 설정을 함께 반영하며 긴 제목과 낮은 창에서 확인한다.

### 2026-09-07: 간트 막대의 이름 배치는 현재 화면 좌표를 따른다

- 날짜 원점에서의 x 좌표만으로 이름을 막대 앞에 둘지 판단하면 전체 일정 맞춤이나 가로 이동 뒤 이름이 화면 왼쪽으로 잘릴 수 있다. 현재 스크롤과 보이는 날짜 영역의 여백을 함께 계산한다.
- 그룹 일정 이동은 렌더링된 행 목록이 아니라 정본의 모든 후손을 대상으로 저장한다. 접힌 항목과 완료 항목도 같은 날짜 차이를 적용하고, 자동 일정 전환은 한 번 확인한다.
- 진행률 효과는 증가 때만 짧게 재생하며 입력 요소를 다시 만들지 않는다. 완료는 숨김·삭제와 별도 상태로 분명히 표시한다.

### 2026-09-07: 저장과 알림 준비를 분리하고 IPC 수신 준비를 확인한다

- 리테이크 저장 전에 필요한 것은 인증된 사용자와 세션의 고정이다. Slack용 사용자 정보 조회가 실패했다는 이유로 본문 저장까지 막지 않는다. 전송용 최신 역할·Slack 정보 조회 실패는 저장 후 알림 결과로 보고한다.
- Electron 창이 존재하거나 로딩을 마쳤다고 renderer의 React 수신 효과까지 준비된 것은 아니다. 딥링크는 실제 리스너 설치 확인을 받은 뒤 전달하고, 새로고침 중에는 보관한다. Windows의 second-instance와 파일 fallback이 같은 실행을 중복 전달할 수 있으므로 실행 ID로 중복만 제거하고 같은 URL의 새 실행은 허용한다.
- 본문 저장 뒤 실행하는 외부 알림도 저장 IPC에서 기다리면 화면 반영을 지연한다. 저장 응답은 즉시 반환하고 알림 결과는 현재 발신자에게 별도로 전달한다.
- 메인 위젯의 최초 데이터 조회는 연결 모드와 로컬 모드 모두 준비한다. 서버 알림 보충 조회의 연결 조건으로 로컬 저장소 초기화까지 건너뛰지 않는다.
- 정확한 항목 바로가기는 캐시의 위치로 먼저 이동하지 않는다. 조회 중 요청과 검증 완료 대상을 분리해야 이미 열린 화면이 요청을 미리 소비하지 않으며, 같은 ID 재요청도 순번으로 구분한다. 단건 검증이 성공한 요청은 전체 목록 조회가 실패해도 열 수 있어야 한다. 진행 중인 정상 목록 조회는 보존하고, 조회가 끝난 뒤 검증된 대상을 반영해 목록 누락과 오래된 대상 덮어쓰기를 막는다.
- 목록의 작은 배지를 카드별 전체 댓글 조회로 구현하면 읽음 처리 한 번에도 조회가 증폭된다. 경량 요약을 묶어서 읽고 댓글 변경과 읽음 상태 변경을 분리하며, DB의 기본 행 제한을 넘는 경우도 검증한다.
- DB 전체 조회 결과에서 ID를 찾지 못했다고 바로 삭제로 판단하면 서버의 기본 행 제한에 잘린 항목을 오판한다. 정확한 대상은 ID 단건 조회를 사용하고 리테이크·세트·댓글·읽음 기록처럼 연결된 전체 목록은 모두 빈 페이지가 반환될 때까지 읽으며, 서버 제한이 요청 개수보다 낮아도 중단하지 않는다. 네트워크 실패와 실제 없음은 구분한다.
- 배치 캐시는 조회 대상을 dirty 목록에서 꺼낸 뒤 실패했을 때 복구 경로가 필요하다. 요약과 읽음 상태 각각에 독립된 재시도를 두고, 활성·최신 대상만 제한된 횟수로 다시 요청하며 화면·계정 전환 이후에는 취소한다.
- DB 요청 성공과 실제 행 수정 성공을 구분한다. 사전 조회 뒤 삭제될 수 있으므로 update 결과의 affected 여부를 renderer까지 전달하고, 미갱신이면 낙관적 상태를 롤백하며 완료 알림을 보내지 않는다.
- 재시도 테스트는 mock의 throw만 확인하지 말고 실제 persistence에서 오류가 helper까지 전달되는지 연결해 확인한다. 하위 서비스가 오류를 빈 결과로 바꾸면 상위 재시도는 실행되지 않으므로 기존 fallback과 엄격한 조회 계약을 분리한다.
- 로그인 대기 중 링크 ID 보존과 검증된 대상 snapshot 보존은 다르다. 계정·연결 전환은 동기 경계에서 검증을 무효화하고 새 요청 순번으로 재조회하며, 늦은 화면 효과도 현재 요청과 일치할 때만 소비한다.

## 2026-09-07: 로그인 오류를 계정 삭제 문제로 바꾸지 않는다

- PC 버전을 사용자가 확인할 수 없어도 코드로 재현 가능한 공통 예방 조치는 진행할 수 있다. 단, 해당 PC의 원인 확정·실제 업데이트 성공과 혼동하지 않는다. 로그인 화면 버전 버튼은 v1.117.1 이후 기능이므로 구버전에도 보인다고 안내하지 않는다.
- 서버를 확인하지 못한 새 로그인은 사용자 명단이나 저장된 옛 비밀번호로 성공·미등록·비밀번호 오류를 결정하지 않는다. 오프라인 로그인 폐지는 사용자 승인을 받은 뒤 적용하며, 기존 계정·설정 파일을 정리하거나 초기화하는 작업과 분리한다.

- “등록되지 않은 사용자”가 표시된다는 신고는 먼저 정확한 오류 문구·설치 버전·원격 조회 결과를 확인한다. 목록에 원치 않는 사용자가 보인다는 뜻으로 단정하지 않는다.
- 초기 비밀번호와 다르다는 사실만으로 계정이 잘못되었다고 판단하지 않는다. 등록 여부·서버 연결 실패·비밀번호 불일치는 분리해서 검증한다.
- 로그인 복구를 위해 계정을 삭제하거나 재생성하지 않는다. 삭제가 정말 필요하면 대상·작업 기록에 미치는 영향·복구 방법을 설명하고 별도 확인을 받는다.
- 컬럼 권한 강화 후 구버전 `select *`는 실패할 수 있다. 앱 업데이트와 로그인 전 복구 경로를 확인하고, 불완전한 로컬 목록으로 미등록을 확정하지 않는다.
...
### 2026-09-17: 캘린더 가져오기에서 컨테이너와 일정 복사를 구분한다

- 공유 캘린더를 간트로 가져온다는 요청을 일정 몇 건의 독립 복사로 좁혀 해석하지 않는다. 캘린더 자체의 폴더·프로젝트 대응, 소유자, 공유 대상, 보기·편집 권한 유지와 이후 자동 연동 여부를 함께 확인한다.
- 원본 연동은 일정뿐 아니라 공유 인원 변경·접근 회수·팀 신규 인원까지 같은 원본 권한을 따르게 한다. 기존에 개별 복사한 항목을 임의로 자동 연동 대상으로 전환하지 않는다.

...
- Electron 창이 존재하거나 로딩을 마쳤다고 renderer의 React 수신 효과까지 준비된 것은 아니다. 딥링크는 실제 리스너 설치 확인을 받은 뒤 전달하고, 새로고침 중에는 보관한다.
...
- DB 전체 조회 결과에서 ID를 찾지 못했다고 바로 삭제로 판단하면 서버의 기본 행 제한에 잘린 항목을 오판한다. 정확한 대상은 ID 단건 조회를 사용하고 전체 목록은 빈 페이지가 반환될 때까지 읽으며, 서버 제한이 요청 개수보다 낮아도 중단하지 않는다. 네트워크 실패와 실제 없음은 구분한다.

...
- DB 전체 조회 결과에서 ID를 찾지 못했다고 바로 삭제로 판단하면 서버의 기본 행 제한에 잘린 항목을 오판한다. 정확한 대상은 ID 단건 조회를 사용하고 전체 목록은 페이지를 모두 읽으며, 네트워크 실패와 실제 없음은 구분한다.
- 배치 캐시는 조회 대상을 dirty 목록에서 꺼낸 뒤 실패했을 때 복구 경로가 필요하다. 활성·최신 대상만 제한된 횟수로 다시 요청하고, 화면·계정 전환 이후의 재시도는 취소한다.

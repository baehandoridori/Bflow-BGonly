...
## 2026-09-06: 테이블 권한 회수는 그 테이블을 건드리는 남의 트리거까지 본다

- 간트 테이블을 anon 에서 차단한 뒤 `users`/`calendars` 삭제가 통째로 막혔다. 두 테이블에 붙은 간트 정리 트리거가 SECURITY INVOKER 라 호출자(anon) 권한으로 간트 테이블을 LOCK 하려 했기 때문이다. 삭제 RPC 도 SECURITY INVOKER 라 권한이 승격되지 않는다.
- 권한을 회수할 때는 `pg_trigger` 에서 **다른 테이블에 붙은 트리거 중 이 테이블을 건드리는 것**을 함께 찾는다. 트리거 함수가 SECURITY INVOKER 면 회수 즉시 그 경로가 죽는다.
- 정리 트리거는 SECURITY DEFINER 가 맞다. 이미 권한 검증을 통과한 삭제의 뒷정리만 하고, `RETURNS trigger` 라 RPC 로 직접 호출되지 않으며, `search_path` 가 고정돼 있다.
- 이 회귀는 테스트·빌드·리뷰·배포를 모두 통과했다. 차단 이후 삭제를 한 번도 시도하지 않았기 때문이다. **권한을 바꾸면 그 테이블을 참조하는 모든 쓰기 경로(특히 삭제)를 롤백 트랜잭션에서 한 번씩 실행해 본다.**
- 컬럼 단위 권한을 적용하기 전에는 배포된 번들에서 실제 조회·쓰기 패턴을 grep 으로 확인한다. `select('*')` 가 하나라도 남아 있으면 그 경로가 통째로 깨진다.

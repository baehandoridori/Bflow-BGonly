...
> **최종 갱신**: 2026-09-14 (v1.118.0 — 팀 할 일 목록·캐릭터 현황판 바로 열기, 피드백 58·59)
...
### 2026-09-20 캘린더 날짜·시간 입력 개선 및 Apple 캘린더 구독 [v1.124.0 배포 완료]

- [x] 앱 전용 날짜 범위 선택, 24시간 입력·15분 제안·기간 버튼을 생성/편집창에 적용.
- [x] 소유자 전용 구독 주소 발급·교체·중지와 읽기 전용 ICS 피드, 프리뷰 관리 기능.
- [x] 권한·구독·입력 검증, 운영 DB/Edge 적용 및 실제 HTTPS 확인, 정식 빌드 2,751개 검사 통과. PR #294 머지 후 manifest 마지막 배포와 7,337개 파일 SHA256 불일치 0개 확인.

### 2026-09-17 공유 캘린더 전체를 간트에 자동 연결 [v1.123.0 배포 완료]

- [x] 캘린더마다 연결 폴더·프로젝트를 만들고 원본 일정 추가·수정·삭제를 자동 반영한다.
- [x] 팀 전체·선택 인원 공유 및 원본 편집 권한 변경을 그대로 반영한다.
- [x] 연결된 일정은 원본 편집창으로 열며, 연결 해제는 원본을 보존한다.
- [x] 테스트 모드·운영 DB rollback probe, 정식 빌드(2,697개 통과), PR #292 머지 및 manifest-last 배포(7,335개 파일 해시 불일치 0)를 완료했다.

### 2026-09-16 캘린더 안정화·관리자 보기·여러 태그·간트 가져오기 [v1.120.0 배포 완료]

- [x] 캘린더·기간·일정을 선택해 간트 프로젝트로 복사하고 중복 가져오기를 방지한다.
- [x] 운영 DB 적용, PR #288 머지, 정식 설치 파일 빌드 및 manifest-last 배포 검증(7,335개 파일 해시 불일치 0).

- 생성 후 실시간 재조회와 확인 요청이 겹칠 때의 잘못된 오류 표시 수정.
- 배한솔 관리자 전용 전체 캘린더 열람, 미공유 목록 기본 접기 및 개별 표시 선택.
- 일정 여러 태그 선택, 관리자 태그 추가·삭제, 태그 색상 배지와 툴팁 표시.
- 서버 세션 권한, 기존 단일 태그 호환, 테스트 모드 동등성 검증.
...
### 2026-09-14 팀 할 일 · 캐릭터 현황판 바로 열기 (v1.118.0, 피드백 58·59) `[배포 완료, 실기 확인 대기]`

- [x] 계획서·실행 프롬프트 (PR #283): 세션 토큰 래퍼·내용 없는 실시간 신호 설계, 커밋 경계 시뮬레이션·독립 리뷰
- [x] 씬·캐릭터 댓글 패널 위 팀 공유 할 일(추가·완료·삭제·접기, 작성자·완료자 표시), 사이드바 '캐릭터' 새 창 버튼 (PR #284)
- [x] 로컬 멀티 에이전트 리뷰(최신 댓글 가림 수정)·뮤테이션 테스트 보강(생존 40종 → 48종 전부 감지), 코덱스 리뷰 루프 11차 명시 완료
- [x] 운영 DB `comment_thread_todos` 적용(20260913225950)·스모크 통과·기존 데이터 지문 동일 — `DEVLOG/2026-09-14-comment-thread-todos-live-application.md`
- [x] 타입 검사·전체 테스트·Vite 빌드·미리보기 동작 확인 — `DEVLOG/2026-09-14-피드백-58-59-구현.md`
- [x] 머지(`1ab7a102`)·정식 빌드·공유 드라이브 배포(manifest 마지막 갱신) — 3파일 SHA-256·버전·파일 수 7,335 원격 일치, 2026-09-14 08:55 KST
- [ ] 실기 확인: 두 PC 실시간 반영·삭제 권한 표시·팝업 창 댓글 토글·재로그인 안내·재연결 따라잡기

### 2026-09-09 서버 전용 새 로그인 `[진행]`

- [x] 사용자 승인: 오프라인 새 로그인 보류, 계정·로컬 데이터 삭제·초기화 금지
- [x] 로컬 비밀번호 인증·미등록 오판 경로 재현 및 서버 전용 판정 수정
- [x] 테스트 모드·개인 데이터 세션 전환 회귀·타입 검사·전체 Vite 빌드 검증 (2,491 통과, 기존 DB 28 건너뜀)
- [ ] 독립 리뷰·PR·병합본 정식 설치 파일 배포 검증
- [ ] 문제 발생 PC의 실제 업데이트 적용 및 재로그인 확인 (중앙 검증과 별도)

### 2026-09-07 로그인 복구 `[배포 완료, 사용자 PC 확인 대기]`

- [x] 서버 조회 실패와 실제 미등록 계정 판정 분리, 기존 인증 경계 보존
- [x] 로그인 전 업데이트 진입·타입 검사·관련 테스트·Vite 빌드·프리뷰 로그인 검증
- [x] PR 리뷰·머지 및 병합본 설치 파일 배포 검증 — v1.117.1, 현재 배포 파일 7,333개 SHA-256 일치
- [ ] 문제 발생 PC에서 v1.117.1 업데이트 후 재로그인 확인

### 2026-09-07 리테이크·캐릭터 피드백 `[구현·로컬 검증 완료, 운영 확인 대기]`

- [x] 캐릭터 카드·리스트에 기존 작은 댓글 배지 연결
- [x] 리테이크 담당 진행·최종 완료 컨트롤 연결
- [x] 두 리테이크 허브 안에서 씬 상세 열기
- [x] 내 리테이크 위젯·대시보드 진행 상태 안내
- [x] 슬랙 담당 알림·요청자 수동 재알림·리테이크 바로가기 구현
- [x] 타입 검사·전체 테스트·Vite 빌드 및 로그인한 프리뷰 동작 확인
- [ ] Slack 워크플로 버튼 템플릿·실수신·설치 앱 바로가기 확인
...
실험 기능: 배플레이그라운드     ████████░░ 배한솔 한정 테스트 (결정론 시세·양방향 빠른주문·조작형 차트)
```
...
> **초기 사용자**: 배한솔 계정만 접근 가능. 다른 팀원에게는 아직 공개하지 않으며, 사이드바와 화면을 모두 숨긴다.
...
- [x] 배한솔 사용자 한정 접근 게이트
- [x] 1초 단위로 움직이는 시세와 선·캔들 차트, 시간 간격·기간 선택
- [x] 정수 주식 수량 주문과 화면에 붙어 따라오는 매수·매도 조작부
- [x] 포인트·증권계좌·보유 종목 영구 저장과 배한솔 최초 백만 포인트 1회 지급
...
- [x] 클래식 2048 실제 플레이 + 방향 이동·합성·단계별 보드 충격 + 포인트·순위표 연결 (v1.90.0): `docs/superpowers/specs/2026-07-14-playground-2048-design.md`
...
- [x] 게임 실행과 서버 저장 → **PR A~D 완료 (v1.84~1.87.0)**: 아케이드 기반(마이그레이션+RPC, 메인 서비스·IPC, 게이트웨이·스토어, 우상단 포인트 배지, 출석·업무 적립), 스네이크·테트리스 실제 플레이, 등급 보상·도전과제, 게임별 순위표, 신기록 슬랙 알림 토글: `docs/superpowers/plans/2026-07-13-baeplayground-arcade-points.md` · 인터랙션 목업: `docs/superpowers/mockups/2026-07-13-baeplayground-arcade-mockup.html`
  ...
  - [x] PR E: 2048 순수 엔진 + 인터랙션 스테이지 + Arcade/Supabase 기록·보상·랭킹 (v1.90.0)
...
> 2026-09-05 · v1.111.0 구현: 기존 진행률 간트/히트맵 라우트를 아래 프로젝트 일정 편집 화면으로 교체. 운영 DB 적용·배포는 별도 단계.

- [x] **개인·공유 프로젝트 간트**: 폴더 공유/멤버 권한, 프로젝트 켜기·끄기, 그룹·작업·마일스톤, 작업자·참석자·씬 연결.
- [x] **직접 편집**: 막대 옆 이름과 총 기간, 격자 없는 날짜축, 커서 중심 휠 확대, 드래그·리사이즈, 우클릭 빠른 편집·색상, 실행 취소/재실행.
- [x] **완료와 일정 계산**: 작업별 자동/수동, 선행 관계, 수동 일정 충돌 표시, 개별·프로젝트 완료/복원, 완료 보기.
- [x] **선택 작업 캘린더 연결**: 제목·날짜·시간·메모 양방향 편집, 양쪽 편집 권한 확인, 연결 해제/삭제 일관성.
- [x] **운영 DB 연결**: 간트 SQL `20260905124058` 적용, 실제 앱 저장 모듈의 HTTP 조회 및 anon 역할의 저장·공유·캘린더 연결 롤백 검증. 기존 데이터·사용자 삭제 함수 보존 확인.
- [x] **앱 릴리스** (v1.112.2): 로컬 심층 리뷰·정식 빌드·공유 드라이브 배포·실제 Windows 설치 및 개인 계정 간트/캘린더 인수 완료. 공유 권한·충돌은 DB/자동 시험·두 preview 탭으로 검증했으며 여러 실계정/PC의 동시 UI 조작은 미실시. 상세 범위는 아래 인수 기록 참조.
- [x] **v1.112.1 릴리스 재검증**: 자동 후속 일정 재계산, 공유 멤버 정리, 연속 실행 취소, 캘린더 권한 및 로그인 정보 저장 보호 보완. 로컬 심층 리뷰 대체 승인, 2,161개 테스트 및 정식 빌드, 운영 DB 보완, 공유 드라이브 7,327개 파일 SHA-256 일치 확인. 실제 PC 적용 상태는 `DEVLOG/2026-09-06-gantt-release-audit.md` 참조.
- [x] **간트·캘린더 상황별 인수 검증** (v1.112.2 완료): 2,199개 자동 시험, 양방향 preview 조작·동시 편집, 운영 rollback RPC·로컬 심층 리뷰 통과. 전체 7,327개 배포 해시와 Windows 설치 적용을 확인했으며, 로그인된 실제 앱에서 개인 작업 저장·캘린더 양방향 수정·0분 마일스톤·삭제/복원/재삭제 및 검증 데이터 정리 완료. `DEVLOG/2026-09-06-gantt-calendar-scenarios.md` 참조.
- [x] **간트 실사용 개선** (v1.113.0): 전체 기본 보기와 완료 유지, 그룹 완료 일관성, 계층 안내·삭제 메뉴, 빈 영역 화면 이동 및 명시적 작업 만들기 모드. 로컬 심층 리뷰, 2,222개 자동 시험과 실제 브라우저 조작 인수 완료. 구현·검증 범위는 `DEVLOG/2026-09-06-gantt-usage-ux.md` 참조.
- [x] **간트 직접 편집과 계층 이동** (v1.114.0): 상세 자동 저장, 숫자·막대 진행률과 차트 표시, 공용 선택창, 폴더 트리, 작업·그룹 순서 및 프로젝트 이동. 구현·검증 범위는 `DEVLOG/2026-09-06-gantt-direct-edit.md` 참조.
- [x] **간트·캘린더 사용성 감사** (v1.115.0 구현·검증 완료, 배포 증거 PR #272): 월 이동·ISO 주차, 연결 색상·0분 마일스톤·보기 전환, 공용 메뉴·툴팁·라이트 모드 개선. `DEVLOG/2026-09-06-gantt-calendar-polish-report.md` 참조.
- [x] **간트 작업 공간 정리** (v1.115.1): 상시 세부정보를 전체 메모 호버로 이동, 그룹 표시와 접기 버튼, 트리·차트 접힘 분리, 상단 밀도와 상세 제목·글자 정렬. 로컬 리뷰·2,333개 시험·preview 검증 완료. `DEVLOG/2026-09-06-gantt-compact-layout.md` 참조.
- [x] **간트 일정 이동·진행 표시** (v1.117.0): 그룹 전체 후손 날짜 이동, 진행률 채움·테두리 반응과 완료 버튼, 새 항목 계층 색상, 막대 전용 전체 메모, 기간·남은 일수와 이름 구분. 로컬 리뷰·작은 창·큰 글씨 검증 완료, 배포 증거는 PR에 기록. `DEVLOG/2026-09-07-gantt-schedule-visibility.md` 참조.
...
- [x] **공유 캘린더 (PM 일정관리)**: 팀 전체/특정 팀원/개인 캘린더 + 태그 필터 + 시간 단위 일정 + 앱 내 알림 (2026-08 라운드, 설계서 docs/superpowers/specs/2026-08-24-calendar-pm-shared-calendars-design.md)
...
### 8-2. 키보드 단축키 커스텀 `P8` `[완료]`

> 난이도: ●●○ | 완료일: 2026-02-27
> 핵심: VS Code 스타일 키바인딩 에디터 — 녹음 모드 + 충돌 감지 + 카테고리별 그룹핑

- [x] **ShortcutsSection.tsx**: 15개 단축키 정의 (탐색/뷰 전환/편집/시스템 4개 카테고리)
- [x] **녹음 모드**: 클릭 → 모달 오버레이 → 키 조합 캡처 → 적용/취소
- [x] **충돌 감지**: 동일 키 조합이 다른 단축키에 이미 할당된 경우 경고
- [x] **검색**: 단축키 이름/설명/키 조합으로 필터링
- [x] **영속화**: preferences.json에 customBindings 저장, 기본값 복원 버튼
- [x] **키 뱃지 UI**: ⌃ ⌥ ⇧ 기호 + 모노스페이스 kbd 스타일
...
- [x] **StartupSection.tsx**: 로딩 스플래시 / 랜딩 스플래시 개별 토글 + 기본값 복원
- [x] **App.tsx 조건 분기**: skipLoadingSplash → setLoadingSplashDone(true), skipLandingSplash → setShowSplash(false)
- [x] **preferences.json 영속화**: 다음 앱 시작 시 적용

### 8-5. 로그인 자동저장 `P8` `[완료]`

> 난이도: ●○○ | 완료일: 2026-02-27

- [x] **LoginSection.tsx**: "로그인 유지" 토글 + 비활성 시 경고 메시지
- [x] **LoginScreen 체크박스**: rememberMe 옵션 (기본값 true — 하위 호환)
- [x] **세션 복원 로직**: App.tsx에서 rememberMe 체크 후 auth.json 복원 여부 결정
- [x] **userService.ts 연동**: login() 시 rememberMe 파라미터로 세션 저장 제어
...
> Phase 1~2 완료, Phase 6 Step 1~4 완료, Phase 7-1~7-5 완료, Phase 8-0~8-1, 8-3~8-5 완료.
...
| **캘린더 위젯** | `CalendarWidget.tsx` (842줄) | 대시보드 미니 캘린더, 이벤트 히트맵, 필터링 | ✅ 완료 |
...
| **팀 할 일** | `ThreadTodoSection.tsx`, `electron/threadTodoStore.ts` | 씬·캐릭터 댓글 스레드별 팀 공유 체크리스트 — 세션 토큰 래퍼 + 내용 없는 실시간 신호 (v1.118.0) | ✅ 완료 |
...
## v1.125.0 반복 일정과 시작 전 알림

- [x] 반복 원본·회차 예외와 범위별 수정/삭제, 장소·회의 URL
- [x] 캘린더·간트 날짜 구간 확장, 외부 표준 구독 반복/예외
- [x] Windows 시작 전 알림, 종일 오전 9시 기준, 최근 30분 catch-up, 중복방지
- [x] 최종 화면·정식 빌드 검증 및 공유 드라이브 배포 — 2026-09-20, 2,831 tests / 7,337 files SHA-256 일치. [검증 기록](docs/verification/2026-09-20-calendar-recurrence-reminders/README.md)

## v1.125.1 캘린더 사이드바 기본 펼침

- [x] 캘린더 첫 진입·재진입 시 사이드바 기본 펼침, 현재 화면에서 수동 접기·펼치기 유지
- [x] 기존 UI 회귀 테스트 및 배한솔 preview 로그인 상태에서 첫 진입·접기·다른 화면 후 재진입 확인

### v1.126.0 공유 캘린더 구독 접근 개선 (2026-09-21)
- [x] 전체 태그 모두 끄기·켜기
- [x] 공유 멤버의 읽기 전용 설정 진입
- [x] 구독 주소 재조회 및 기존 구독 호환·서버 검증
- [x] 정식 빌드·배포 검증 (v1.126.0, 7,337파일 SHA256 일치)
- [x] Ctrl/Meta 씬 카드 클릭 중 작은 움직임으로 선택이 초기화되는 문제 수정

### v1.127.2 씬 카드 Shift 범위 선택 (2026-09-21)

- [x] 통합·BG·ACT 카드의 Shift 연속 선택과 레이아웃 그룹 간 범위 처리
- [x] 정렬·필터 변경 시 씬 식별자 기준 앵커 유지와 Ctrl 선택 회귀 검증
- 배포 증거: `output/release-audits/2026-09-22-v1.127.2/`

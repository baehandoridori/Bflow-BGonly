...
> **최종 갱신**: 2026-02-24 (Phase 0 긴급 안정화 추가)
> **프로젝트**: Bflow (Studio JBBJ 프로덕션 진행 현황 대시보드)
> **담당**: Claude × 한솔 (Studio JBBJ)
> **현재 상태**: Phase 1~2 완료, Phase 4-1~4-3 완료, Phase 6 Step 1~4 완료, Phase 7-1~7-5 완료, **Phase 0 긴급 안정화 착수**
...
```
Phase 0: 긴급 안정화 (신규)    ░░░░░░░░░░ 착수 예정 ⚡ [최우선]
Phase 1: 핵심 씬 관리         ██████████ 완료 ✅
Phase 2: 이미지 리뷰           ██████████ 완료 ✅
Phase 3: 알림 & 협업           ░░░░░░░░░░ 미착수
Phase 4: 뷰 확장               ██████░░░░ 4-1~4-3 완료 ✅ (4-2 간트, 4-4 대기)
Phase 5: 커스터마이징           ░░░░░░░░░░ 미착수
Phase 6: 멀티 부서 확장        ████████░░ Step 1~4 완료 ✅ (Step 5 대기)
Phase 7: 대시보드 UX 혁신      ████████░░ 7-1~7-5 완료 ✅ (7-2 AOT 대기)
```

---

## Phase 0: 긴급 안정화 — Google Sheets 연동 신뢰성 `[다음]` `[최우선]`
...
### 0-1. Apps Script 배치(Batch) 엔드포인트 `P0` `[다음]` `[최우선]`

> 난이도: ●●○ | 예상 효과: 부분 실패 완전 해결 + 속도 3배 이상 향상

**현재 문제:**
```
파트 3개 추가 요청 시:
  [앱] → HTTP 1 → [GAS] → 시트 탭 생성 → OK  (1~3초)
  [앱] → HTTP 2 → [GAS] → 시트 탭 생성 → OK  (1~3초)
  [앱] → HTTP 3 → [GAS] → 시트 탭 생성 → FAIL ❌
  결과: 2개만 생성됨 → UI에선 3개 보이다가 → 동기화 시 2개로 롤백
```

**해결 방안:**
```
변경 후:
  [앱] → HTTP 1 (배치) → [GAS] → 3개 탭 모두 생성 or 모두 실패 → 응답  (3초)
  결과: 전부 성공 or 전부 실패 (원자적 처리)
```

- [ ] **GAS `batch` action 추가**: `doGet`/`doPost`에 `action=batch` 핸들러
  - 요청 바디: `{ actions: [{ action: 'addPart', params: {...} }, ...] }`
  - 단일 스크립트 실행에서 모든 작업 순차 처리
  - 하나라도 실패 시 전체 에러 반환 (부분 성공 방지)
  - 각 action의 결과를 배열로 반환
- [ ] **클라이언트 `gasBatch()` 함수 추가**: `electron/sheets.ts`에 배치 전송 헬퍼
  - 기존 개별 함수들은 유지 (단일 동작은 그대로)
  - 복수 동작 시에만 배치로 묶어서 전송
- [ ] **에피소드 추가 배치 적용**: BG+ACT 파트를 한 번의 배치로 생성
  - 현재: `addEpisode(bg)` → `addEpisode(act)` → `addPart(A,bg)` → `addPart(A,act)` (4번 요청)
  - 변경: `batch([addEpisode(bg), addEpisode(act), addPart(A,bg), addPart(A,act)])` (1번 요청)
- [ ] **파트 추가 배치 적용**: 동일 에피소드에 BG+ACT 파트 동시 생성
- [ ] **기존 개별 API 하위 호환 유지**: batch가 아닌 기존 요청도 정상 동작
...
### 0-2. 낙관적 업데이트 롤백 보강 `P0`

> 난이도: ●○○ | 배치 엔드포인트와 함께 적용하면 효과적

**문제**: 아카이브 등 복합 동작에서 서버 저장 실패 시 UI 낙관적 상태가 완전히 롤백되지 않음.
```
현재 아카이브 흐름 (ScenesView.tsx:1882~1934):
  ① deleteEpisodeOptimistic(epNum)          ← 활성 목록에서 즉시 제거
  ② setArchivedEpisodes([...prev, 추가])    ← 아카이브 목록에 즉시 추가
  ③ writeMetadata('title') → writeMetadata('archive-info') → archiveEpisodeInSheets()
  ④ catch: alert() + syncInBackground()     ← ❌ ②의 archivedEpisodes는 롤백 안 됨!

  결과: 실패 시 에피소드가 활성 목록(sync으로 복원) + 아카이브 목록(유령) 양쪽에 표시
```

- [ ] **아카이브 실패 시 archivedEpisodes 롤백**: catch에서 이전 상태 복원
- [ ] **삭제/수정 등 다른 낙관적 동작도 롤백 점검**: 동일 패턴 적용
- [ ] **배치 엔드포인트(0-1)와 함께 적용**: 아카이브 3단계 호출을 1번 배치로 묶으면 부분 실패 자체가 제거됨

### 0-3. 클라이언트 재시도(Retry) 로직 `P0`

> 난이도: ●○○ | 네트워크 일시 오류 대응

- [ ] **자동 재시도**: HTTP 요청 실패 시 최대 2회 재시도 (지수 백오프: 1초, 3초)
  - 5xx 서버 에러, 네트워크 타임아웃만 재시도
  - 4xx 클라이언트 에러는 즉시 실패 처리
- [ ] **재시도 중 UI 표시**: 토스트로 "동기화 재시도 중..." 표시
...
### 0-5. 종료 시 미완료 배치 큐 보장 `P0`

> 난이도: ●●○ | 대량 삭제/수정 중 앱 종료 시 데이터 일관성 보장
...
- [ ] **배치 큐 매니저**: main 프로세스에 미완료 작업 큐 관리
  - 청크 전송 전 큐에 등록, 성공 시 제거
  - `app.on('before-quit')` 훅에서 미완료 큐 처리
- [ ] **종료 보류 UI**: "저장 중..." 스플래시 or 트레이 알림 표시
- [ ] **(선택) 큐 영속화**: `%APPDATA%/Bflow-BGonly/pending-queue.json`으로 크래시 대비
- [ ] **(선택) 재시작 시 큐 복원**: startup 시 큐 파일 확인 → 이어서 처리

```
구현 순서:
  Step 1: GAS batch action 구현 + 재배포 ← 최우선
  Step 2: 클라이언트 gasBatch() 함수 + 에피소드/파트 추가에 적용
  Step 3: 낙관적 업데이트 롤백 보강 (아카이브 등 복합 동작)
  Step 4: 재시도 로직 추가
  Step 5: (선택) 테스트 모드 제거 검토
  Step 6: 종료 시 미완료 배치 큐 보장
```

### 0-6. 위젯 팝업 위치/크기 저장 및 복원 `P1`

> 난이도: ●●○ | 앱 재시작 시 위젯 팝업 위치/크기/AOT 상태 유지
...
- [ ] **위젯 상태 저장**: move/resize 이벤트 디바운스로 JSON 파일 업데이트
- [ ] **앱 시작 시 복원**: main 프로세스에서 JSON 읽어 팝업 자동 생성
- [ ] **화면 밖 보정**: 저장된 좌표가 현재 모니터 범위 밖이면 가시 영역으로 이동
...
> **2026-02-24 기준**: Phase 0(긴급 안정화)를 최우선으로 진행.
> Phase 1~2 완료, Phase 6 Step 1~4 완료, Phase 7-1~7-5 완료.
> Phase 3~5는 Phase 0 이후 또는 필요시 병행.

### 최우선 — Phase 0 (긴급 안정화)

| 순위 | 항목 | 세부 | 난이도 | 상태 |
|------|------|------|--------|------|
| **1** | **0-1. 배치 엔드포인트** | GAS batch action + 클라이언트 gasBatch() | ●●○ | 🔜 다음 |
| **2** | **0-2. 낙관적 업데이트 롤백 보강** | 아카이브 등 복합 동작 실패 시 UI 복원 | ●○○ | 대기 |
| **3** | **0-3. 재시도 로직** | HTTP 요청 실패 시 자동 재시도 | ●○○ | 대기 |
...
| 순위 | 항목 | 페이즈 | 난이도 | 비고 |
|------|------|--------|--------|------|
| 4 | 동기화 최적화 | P5 | ●●○ | 폴링 주기 + diff 기반 업데이트 (P0과 시너지) |
| 5 | 태그/확인요청 버튼 | P3 | ●●○ | 협업 핵심 |
| 6 | 슬랙 웹훅 연동 | P3 | ●●● | 알림 자동화 |
| 7 | 커스텀 파이프라인 단계 | P5 | ●●● | 부서별 단계 커스텀 |
| 8 | 메인 대시보드 강화 | P4 | ●●○ | 세부 진행률 |
| 9 | AOT 플로팅 위젯 | P7 | ●●● | 글래스 효과 |
| 10 | 컴포지팅 확장 | P6 | 미정 | 6-5 향후 결정 |

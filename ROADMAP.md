...
> **최종 갱신**: 2026-02-20 (Phase 2 완료, Phase 6~7 아이디어 추가)
...
## Phase 6: 멀티 부서 확장 — BG + 액팅 + 컴포지팅 `[중요]`
...
```
구현 우선순위:
  1단계: 부서 개념 도입 + 시트 구조 확장 (핵심 아키텍처)
  2단계: 액팅 씬 관리 (BG 로직 확장)
  3단계: 대시보드 멀티 부서 위젯
  4단계: 컴포지팅 (필요 시)
```
...
| 16 | **부서 개념 도입 (BG+액팅)** | **P6** | **●●●** | **⚠️ 핵심 아키텍처 확장** |
| 17 | **데이터 소스 구조 설계** | **P6** | **●●○** | **시트 이름 확장, 하위 호환** |
| 18 | **대시보드 멀티 부서 위젯** | **P6** | **●●○** | **BG+액팅+통합 진행률** |
| 19 | **액팅 씬 관리** | **P6** | **●●○** | **BG 로직 확장 재사용** |
| 20 | 위젯 레이아웃 편집 (iPhone wiggle) | P7 | ●●● | 대시보드 커스터마이징 |
| 21 | AOT 플로팅 위젯 (글래스 효과) | P7 | ●●● | 바탕화면 현황 위젯 |
| 22 | 진행률 동기부여 메시지 | P7 | ●○○ | 재미 요소 |
| 23 | 전체 UI 통일성 & 폴리시 | P7 | ●●○ | 시각적 완성도 |
| 24 | 스포트라이트 검색 (Ctrl+Space) | P7 | ●●● | 빠른 네비게이션 |

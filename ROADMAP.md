...
## Phase 1: 핵심 씬 관리 강화 (현재 → 다음 릴리즈)

> **목표**: 현재 씬 목록 뷰의 가시성과 조작성을 프로덕션 수준으로 끌어올린다.
> **영향도**: 매일 쓰는 핵심 워크플로우 직접 개선. 체감 효과 즉시.

### 1-1. 씬 목록 정렬 및 가시성 개선 `P1`

> 난이도: ●●○ | 참고: ftrack 멀티레벨 그룹핑, Kitsu 상태 매트릭스

- [ ] **정렬 옵션 드롭다운**: 씬번호순 (기본), 담당자순, 진행률순, 미완료 우선
- [ ] **정렬 방향 토글**: 오름차순 ↑ / 내림차순 ↓
- [ ] **상태 필터 칩**: "미착수", "진행중", "완료" 등 빠른 필터 (담당자 필터와 병렬)
- [ ] **컬러 코딩 강화**: 진행률에 따른 카드 좌측 보더 컬러 (0%=빨강, 50%=노랑, 100%=초록)
- [ ] **리스트 뷰 / 카드 뷰 토글**: 카드(현재) + 테이블(밀집 정보) 전환
...
- [ ] **레이아웃 번호 필드 추가**: Scene 타입에 `layoutId: string` 필드 추가
  - 입력 형식: 쉼표 구분 (`a001, a008, a009`) — 스페이스는 무시
  - 각 씬번호는 클릭 가능한 태그로 표시
- [ ] **레이아웃별 뷰 (기본 뷰)**:
  - 레이아웃 번호로 그룹핑 → 각 그룹 안에 해당 씬 카드 나열
  - 레이아웃 헤더에 해당 그룹 진행률 표시
  - 예: `레이아웃 #1 — a001, a004 (진행률 75%)`
- [ ] **씬번호별 뷰**: 기존 씬번호 순서대로 플랫하게 나열 (현재 방식)
- [ ] **뷰 전환 토글**: 상단 바에 `레이아웃별` / `씬번호별` 탭
...
- [ ] **접두사 드롭다운**: `a` / `b` / `c` / `d` / `sc` 등 알파벳 프리픽스 선택
- [ ] **자동 번호 제안**: 선택된 프리픽스의 기존 번호 중 빈 번호를 자동 입력
  - 예: a001, a002, a004 존재 시 → 다음 제안값 `a003`
- [ ] **수동 번호 입력**: 드롭다운 옆 숫자 입력 필드 (제안값이 기본 채워짐)
- [ ] **중복 검사**: 이미 존재하는 씬번호 입력 시 경고

### 1-4. 버전 표기 시스템 `P1`

> 난이도: ●○○ | 빠른 구현 가능

- [ ] **package.json 버전 활용**: `version` 필드를 앱 전역에서 참조
- [ ] **사이드바 하단 표기**: `[TEST]` 모드 표시 아래에 `v1.2.0` 표기
...
## Phase 2: 이미지 확인 및 리뷰 시스템

> **목표**: BG 작업 특성상 이미지 확인이 필수. 씬 카드에서 바로 이미지를 보고 비교할 수 있게.
> **영향도**: BG 아티스트와 감독의 리뷰 워크플로우 혁신.

### 2-1. 씬 이미지 업로드 및 미리보기 `P2`

> 난이도: ●●● | 참고: Kitsu 클립보드 붙여넣기, ftrack 드래그&드롭

- [ ] **이미지 타입 2종**: 스토리보드 이미지 + 가이드(예시) 이미지
- [ ] **업로드 방식 3종**:
  - 클립보드 붙여넣기 (`Ctrl+V`) — BG 아티스트가 DCC 도구에서 캡처 후 바로 붙여넣기
  - 드래그 & 드롭 — 파일을 카드 위에 드래그
  - 파일 선택 다이얼로그 — 클릭하여 파일 브라우저 열기
- [ ] **씬 카드 썸네일**: 작은 미리보기 표시 (좌: 스토리보드, 우: 가이드)
- [ ] **확대 보기**: 클릭 시 모달로 원본 크기 이미지 표시
- [ ] **이미지 저장 전략**:
  - Google Drive 폴더에 업로드 (GAS 경유)
  - URL을 시트의 `스토리보드URL` / `가이드URL` 열에 저장
  - 로컬 캐시: `%APPDATA%/Bflow-BGonly/cache/` (빠른 재로딩)

```
UX 참고:
- Kitsu: 클립보드에서 직접 붙여넣기 지원 (ShotGrid에는 없는 킬러 기능)
- ftrack: 같은 파일명 재업로드 시 버전 자동 증가
- ShotGrid: 드래그&드롭 → 업로드 진행 배너 표시
```
...
- [ ] **사이드 바이 사이드**: 스토리보드 vs 가이드 이미지 병렬 표시
- [ ] **오버레이 비교**: 두 이미지를 겹쳐 놓고 불투명도 슬라이더로 전환
- [ ] **버전 비교**: 같은 씬의 이전 버전과 현재 버전 비교
- [ ] **확대/축소**: 마우스 휠 줌 + 드래그 패닝

### 2-3. 진행률 100% 완료 인터랙션 `P2`

> 난이도: ●○○ | 참고: Asana 완료 축하 애니메이션

- [ ] **씬 완료 애니메이션**: 4단계 모두 체크 시 카드에 짧은 파티클/컨페티 효과
- [ ] **파트 완료 축하**: 파트 내 모든 씬 완료 시 프로그레스 바에 빛나는 효과
- [ ] **에피소드 완료 마일스톤**: 에피소드 전체 완료 시 화면 전체 축하 애니메이션
- [ ] **설정에서 토글 가능**: 개인 설정에서 on/off (기본: on)

```
UX 참고:
- Asana: 선택 가능한 축하 크리처 (유니콘, 나르왈 등), 비차단 오버레이
- 핵심 원칙: (1) 선택적 (2) 드물게 (3) 비차단 (4) 팀 단위 마일스톤에서 더 크게
```
...
| 순위 | 항목 | 페이즈 | 난이도 | 비고 |
|------|------|--------|--------|------|
| 1 | 버전 표기 시스템 | P1 | ●○○ | 빠른 구현 |
| 2 | 씬 목록 정렬/필터 개선 | P1 | ●●○ | 일상 UX 개선 |
| 3 | 씬번호 입력 개선 | P1 | ●●○ | 오타 방지 |
| 4 | 레이아웃별 씬 그룹핑 | P1 | ●●● | BG 워크플로우 핵심 |
| 5 | 이미지 업로드/미리보기 | P2 | ●●● | BG 리뷰 필수 |
| 6 | 진행률 100% 애니메이션 | P2 | ●○○ | 즐거움 요소 |
| 7 | 이미지 비교 뷰 | P2 | ●●○ | 리뷰 품질 향상 |
